...
export default defineEventHandler(async (event) : { data: ProjectT } => {
	...
	const { categoryUuid, title, description, image, softCap, hardCap, finishesAt } = await readBody(event)
	console.log( { categoryUuid, title, description, image, softCap, hardCap, finishesAt })
